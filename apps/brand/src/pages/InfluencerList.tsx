    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex gap-2.5 flex-wrap items-center animate-pulse">
          <div className="h-9 flex-1 min-w-[200px] bg-gray-100 rounded-xl" />
        ...
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden animate-pulse">
          ...
          {[0, 1, 2, 3, 4].map(i => (
            <div key={i} className="flex items-center gap-4 px-4 py-3 border-b border-gray-50">
              ...
              <div className="h-4 w-24 bg-gray-100 rounded ml-2" />
              <div className="h-4 w-12 bg-gray-100 rounded ml-2" />
              <div className="h-4 w-12 bg-gray-100 rounded ml-2" />
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 @md:grid-cols-3 gap-3">
        {summaryStats.map(stat => (
          <div key={stat.label} className="bg-white rounded-xl border border-gray-100 shadow-sm px-4 py-3">
            <p className="text-[15px] text-gray-500">{stat.label}</p>
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 text-[15px] border border-gray-200 rounded-xl bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-green/50 focus-visible:border-brand-green transition-all duration-150"
            />
            {searchInput && (
              <button type="button"
          ...
          <button type="button"
            ...
            className="shrink-0 px-4 py-2 bg-brand-green text-white text-[15px] font-medium rounded-xl hover:bg-brand-green-hover transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-green/50"
          >
            검색
          </button>
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[15px] font-semibold text-gray-900 break-words">@{inf.instagramId ?? inf.name}</p>
                  ...
                  <div className="flex flex-wrap gap-1 mb-3">
                    {inf.category.map(c => (
                      <span key={c} className="text-[15px] bg-gray-100 text-gray-600 px-2.5 py-1 rounded-full">{c}</span>
                    ))}
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center mb-3">
                    <div>
                      <p className="text-[15px] text-gray-500">팔로워</p>
                      <p className="text-[15px] font-semibold text-gray-900">{formatFollowers(inf.followers)}</p>
                    </div>
                    <div>
                      <p className="text-[15px] text-gray-500">참여율</p>
                      <p className={`text-[15px] font-semibold ${getEngagementColor(inf.engagement)}`}>{inf.engagement}%</p>
                    </div>
                    <div>
                      <p className="text-[15px] text-gray-500">진성비율</p>
                      <p className={`text-[15px] font-semibold ${getAuthenticColor(inf.authentic)}`}>{inf.authentic}%</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between flex-wrap gap-y-1.5">
                    <span className="text-[15px] text-gray-500">{inf.platform} · {inf.lastActive}</span>
                    {proposedSet.has(inf.id) ? (
                      <span className="text-[15px] text-gray-500 bg-gray-50 border border-gray-100 px-2.5 py-1 rounded-lg whitespace-nowrap">제안 완료</span>
                    ) : proposableCampaigns.length === 0 ? (
                      <span className="text-[15px] text-gray-500 bg-gray-50 border border-gray-200 px-2.5 py-1 rounded-lg cursor-not-allowed whitespace-nowrap">제안하기</span>
                    ) : (
                      <button type="button"
                        onClick={e => { e.stopPropagation(); setSelectedInfluencer(inf); setProposalModal(true) }}
                        className="text-[15px] border border-gray-200 text-gray-600 px-2.5 py-1 rounded-lg hover:border-gray-400 hover:text-gray-900 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-green/50 whitespace-nowrap"
                      >제안하기</button>
                    )}
                  </div>
                </button>
          ...
          <thead>
            <tr className="bg-gray-50/50 border-b border-gray-100">
              {[
                ...
                <th key={h} scope="col" className={`text-left text-[15px] font-medium text-gray-500 py-3 px-4 whitespace-nowrap ${cls}`}>{h === '액션' ? <span className="sr-only">액션</span> : h}</th>
              ))}
            </tr>
          </thead>
              ...
              >
                ...
                <td className="py-3 px-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <span className="block text-[15px] font-semibold text-gray-900 whitespace-nowrap">@{inf.instagramId ?? inf.name}</span>
                      <span className="block text-[15px] text-gray-500">{inf.name}</span>
                    </div>
                    <button type="button"
                      onClick={e => { e.stopPropagation(); toggleBookmark(inf.id) }}
                      aria-label={bookmarked.has(inf.id) ? '찜 해제' : '찜하기'}
                ...
                <td className="py-3 px-4 hidden @md:table-cell">
                  <div className="flex gap-1 flex-wrap">
                    {inf.category.map(c => (
                      <span key={c} className="text-[15px] bg-gray-100 text-gray-600 px-2.5 py-1 rounded-full whitespace-nowrap">{c}</span>
                    ))}
                  </div>
                </td>

                {/* 팔로워 */}
                <td className="py-3 px-4 text-[15px] text-gray-700 whitespace-nowrap">{formatFollowers(inf.followers)}</td>

                {/* 게시물수 */}
                <td className="py-3 px-4 text-[15px] text-gray-700 whitespace-nowrap hidden @lg:table-cell">{inf.posts.toLocaleString()}</td>

                {/* 평균 좋아요 */}
                <td className="py-3 px-4 text-[15px] text-gray-700 whitespace-nowrap hidden @lg:table-cell">{formatFollowers(inf.avgLikes)}</td>

                {/* 평균 댓글 */}
                <td className="py-3 px-4 text-[15px] text-gray-700 whitespace-nowrap hidden @xl:table-cell">{inf.avgComments.toLocaleString()}</td>

                {/* 참여율 */}
                <td className="py-3 px-4 text-[15px] font-medium whitespace-nowrap">
                ...
                <td className="py-3 px-4 hidden @lg:table-cell whitespace-nowrap">
                  <span className={`text-[15px] font-medium ${getAuthenticColor(inf.authentic)}`}>
                    {inf.authentic}%
                  </span>
                </td>

                {/* 최근 활동 */}
                <td className="py-3 px-4 text-[15px] text-gray-500 whitespace-nowrap hidden @xl:table-cell">{inf.lastActive}</td>
                ...
                <td className="py-3 px-4">
                  {proposedSet.has(inf.id) ? (
                    <span className="text-[15px] border border-gray-100 text-gray-400 px-3 py-1.5 rounded-xl bg-gray-50 cursor-not-allowed">
                      제안 완료
                    </span>
                  ) : proposableCampaigns.length === 0 ? (
                    <Tooltip content="진행 중인 캠페인이 없습니다. 캠페인을 먼저 등록해주세요." multiline>
                      <button type="button"
                        disabled
                        className="text-[15px] border border-gray-200 text-gray-400 px-3 py-1.5 rounded-xl bg-gray-50 cursor-not-allowed whitespace-nowrap"
                      >
                        제안하기
                      </button>
                    </Tooltip>
                    ...
                    <button type="button"
                      ...
                      className="text-[15px] border border-gray-200 text-gray-600 px-3 py-1.5 rounded-xl hover:border-gray-400 hover:text-gray-900 transition-colors duration-150 disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
                    >
                      제안하기
                    </button>
                  )}
      ...
      {selectedInfluencer && !proposalModal && (() => {
        ...
        return (
          ...
          >
            <div
              className={`bg-white shadow-2xl w-full flex flex-col ${device === 'phone' ? 'h-full rounded-none' : 'rounded-2xl max-w-7xl mx-4'}`}
            ...
            >
              ...
              <div className="shrink-0 px-6 pt-5 pb-0">
                <div className="flex items-start gap-3 mb-3">
                  <div className={`w-14 h-14 rounded-full ${AVATAR_COLORS[inf.id % AVATAR_COLORS.length]} flex items-center justify-center text-gray-700 font-bold text-2xl shrink-0`}>
                    {(inf.instagramId ?? inf.name)[0]}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap pr-1">
                      ...
                      <span className="text-[15px] bg-brand-green-bg text-brand-green-text px-2.5 py-1 rounded-full">{inf.type}</span>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {inf.instagramId ? (
                        <a href={`https://instagram.com/${inf.instagramId}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-0.5 text-[15px] text-brand-green-text hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-green/50 rounded">
                          <ExternalLink size={12} aria-hidden="true" />인스타 바로가기
                        </a>
                      ) : (
                        <Tooltip content="인스타그램 username이 등록되지 않았습니다." multiline>
                          <button type="button" disabled className="flex items-center gap-0.5 text-[15px] text-gray-500 cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-green/50">
                            <ExternalLink size={12} aria-hidden="true" />인스타 바로가기
                          </button>
                        </Tooltip>
                      )}
                ...
                <div className="space-y-5">
                  ...
                  <div className="border border-gray-100 rounded-xl p-4">
                    ...
                    <div className="flex gap-1.5 flex-wrap mb-3">
                      {inf.category.map(c => (
                        <span key={c} className="text-[15px] bg-gray-100 text-gray-500 px-2.5 py-1 rounded-full">#{c}</span>
                      ))}
                    </div>
                        ...
                        return (
                          <div key={globalIdx} className="rounded-xl overflow-hidden border border-gray-100 cursor-pointer hover:shadow-md transition-shadow duration-150"
                            ...
                            <div className={`relative overflow-hidden bg-gray-100 ${isFeed ? 'aspect-square' : 'aspect-[9/16]'}`}>
                              ...
                              {!isFeed && <span className="absolute top-1.5 right-1.5 text-[15px] bg-black/55 text-white px-2 py-0.5 rounded-md font-medium">릴스</span>}
                            </div>
                            <div className="px-2 py-1.5 bg-white flex gap-2">
                              <span className="flex items-center gap-0.5 text-[15px] text-gray-500"><Heart size={12} className="text-red-500" aria-hidden="true" />{c.likes.toLocaleString()}</span>
              ...
              <div className="border-t border-gray-100 px-6 py-4 shrink-0">
                {proposedSet.has(inf.id) ? (
                  ...
                  <div className="w-full space-y-2">
                    <Tooltip content="진행 중인 캠페인이 없습니다. 캠페인을 먼저 등록해주세요." multiline>
                      <button type="button" disabled className="w-full bg-brand-green text-white text-[15px] py-3 rounded-xl font-medium opacity-50 cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-green/50">
                        캠페인 제안보내기
                      </button>
                    </Tooltip>
                  ...
                  <button type="button" onClick={() => setProposalModal(true)} className="w-full bg-brand-green text-white text-[15px] py-3 rounded-xl hover:bg-brand-green-hover transition-colors duration-150 font-medium">
                    캠페인 제안보내기
                  </button>
                )}
              </div>
          ...
          <div className="space-y-4">
            <div className={`bg-gray-100 rounded-xl overflow-hidden relative ${contentDetail.type === 'feed' ? 'aspect-square' : 'aspect-[9/16] max-h-[420px] mx-auto'}`}>
              ...
              {contentDetail.type === 'reels' && (
                <span className="absolute top-2 right-2 text-[15px] bg-black/60 text-white px-2.5 py-1 rounded-full font-medium">릴스</span>
              )}
            </div>
            <div className="grid grid-cols-2 gap-2">
        ...
        footer={!proposalSent ? (
          <>
            <button type="button"
              ...
              className="flex-1 border border-gray-200 text-gray-700 py-2 rounded-xl text-[15px] hover:bg-gray-50 transition-colors duration-150"
            >
              취소
            </button>
            <button type="button"
              onClick={handleProposal}
              className="flex-1 bg-brand-green text-white py-2 rounded-xl text-[15px] hover:bg-brand-green-hover transition-colors duration-150"
            >
              제안 보내기
            </button>
          </>
              ...
              {proposableCampaigns.map(c => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {/* 라디오는 visual cue — 카드 전체 클릭으로 선택·펼치기 동시 처리 */}
                      <input
                        type="radio"
                        name="campaign"
                        value={c.id}
