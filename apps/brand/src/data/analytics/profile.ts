...
// ── AI 프로필 분석 더미 — 분석/가이드 그룹 분리 패턴 (광고주 결정 v3)
export const PROFILE_AI_DATA = {
  analysis: [
    '팔로워 수는 소폭 증가했으나 도달·노출·참여율은 감소 추세 — 콘텐츠 반응이 약화되고 있습니다.',
    '프로필 방문과 웹사이트 클릭은 증가하고 있어 *유입 단계는 정상*. 다만 실제 참여로 이어지지 않습니다.',
    '참여율이 0.3%로 업계 평균 대비 낮은 수준 — 팔로워와의 소통 강화가 필요합니다.',
    '팔로워 주요 연령대가 25-34세로 명확하고, 타겟층이 주로 한국에 집중되어 있습니다.',
    '콘텐츠 성과 데이터가 부족해 어떤 유형이 효과적인지 분석에 한계가 있습니다.',
  ],
  guides: [
    '25-34세 타겟에 맞춘 트렌디한 주제 + 퀴즈·댓글 유도형 포스팅 비중을 확대합니다.',
    '정기 스토리·라이브 방송으로 팔로워와의 소통을 강화합니다.',
    '콘텐츠 유형을 다양화하고 성과 기록을 축적해 유형별 효율 데이터를 확보합니다.',
  ],
} as const

// 하위 호환 — 다른 페이지에서 단일 summary string 참조 시 사용
export const PROFILE_AI_SUMMARY = PROFILE_AI_DATA.analysis.join('\n')
