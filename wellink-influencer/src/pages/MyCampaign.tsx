...
export default function MyCampaign() {
  ...
  const handleContentSubmit = () => {
    if (!contentUrl.trim()) { addToast('콘텐츠 URL을 입력해 주세요', 'error'); return }
    try { const u = new URL(contentUrl); if (!/^https?:/.test(u.protocol)) throw new Error() } catch { addToast('유효한 URL을 입력해 주세요 (http/https)', 'error'); return }
    if (submitModal) {
  ...
  return (
    <Layout>
      ...
      <Modal isOpen={!!submitModal} onClose={() => { setSubmitModal(null); setContentUrl('') }} title="콘텐츠 제출">
        <div className="space-y-4">
      ...
      {/* 취소 확인 모달 */}
      <Modal isOpen={!!cancelModal} onClose={() => setCancelModal(null)} title="신청 취소">
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
